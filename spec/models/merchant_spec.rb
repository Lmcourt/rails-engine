...
RSpec.describe Merchant do
  ...
  describe 'methods' do
    before :each do
      ...
      @ii_14 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_14.id, quantity: 1, unit_price: 1, created_at: "2012-03-27 14:54:09")
      ...
      @ii_23 = InvoiceItem.create!(invoice_id: @invoice_2.id, item_id: @item_23.id, quantity: 1, unit_price: 2, created_at: "2012-03-29 14:54:09")

      @ii_3 = InvoiceItem.create!(invoice_id: @invoice_3.id, item_id: @item_3.id, quantity: 1, unit_price: 3, created_at: "2012-03-28 14:54:09")
      @ii_32 = InvoiceItem.create!(invoice_id: @invoice_3.id, item_id: @item_32.id, quantity: 1, unit_price: 4, created_at: "2012-03-28 14:54:09")
    ...
    it 'finds a quantity of merchants sorted by descending revenue' do
      expect(Merchant.ordered_total_revenue(4)).to eq([@merchant3, @merchant2, @merchant1, @merchant4])
    end

    it 'finds a specific merchants revenue' do
      # result = @merchant1.merchant_total_revenue
      # require "pry"; binding.pry
      expect(@merchant1.merchant_total_revenue).to eq(4)
    end

    it 'finds merchants with the most items sold desc' do
      expect(Merchant.most_items(3)).to eq([@merchant1, @merchant2, @merchant3])
    end
  end
end
